...
func (w *Worker) Recommend(users []data.User) {
	...
	err = parallel.Parallel(len(users), w.jobs, func(workerId, jobId int) error {
		...
		for category, result := range results {
			...
			if err = w.CacheClient.SetSorted(ctx, cache.OfflineRecommend, cache.Key(userId, category), results[category]); err != nil {
				log.Logger().Error("failed to cache recommendation", zap.Error(err))
				return errors.Trace(err)
			}
			time.Sleep(w.Config.Recommend.CacheWriteSleep)
...
func (w *Worker) collaborativeRecommendBruteForce(userId string, itemCategories []string, excludeSet *strset.Set, itemCache *ItemCache) (map[string][]string, time.Duration, error) {
	...
	for category, recItemsFilter := range recItemsFilters {
		...
		if err := w.CacheClient.SetSorted(ctx, cache.CollaborativeRecommend, cache.Key(userId, category), cache.CreateScoredItems(recommendItems, recommendScores)); err != nil {
...
func (w *Worker) collaborativeRecommendHNSW(rankingIndex *search.HNSW, userId string, itemCategories []string, excludeSet *strset.Set, itemCache *ItemCache) (map[string][]string, time.Duration, error) {
	...
	for category, catValues := range values {
		...
		recommend[category] = recommendItems
		if err := w.CacheClient.SetSorted(ctx, cache.CollaborativeRecommend, cache.Key(userId, category),
...
func (w *Worker) checkRecommendCacheTimeout(ctx context.Context, user *data.User, categories []string) bool {
	...
	if activeTime.After(recommendTime) {
		timeoutTime := recommendTime.Add(w.Config.Recommend.Offline.CheckRecommendPeriod)
		if timeoutTime.Before(time.Now()) {
			log.Logger().Error("active time timeout", zap.String("user", userId),
				...
				zap.Time("recommendTime", recommendTime))
		}
		return timeoutTime.Before(time.Now())
	}

	// check time
	if modifiedTime.After(recommendTime) {
		timeoutTime := recommendTime.Add(w.Config.Recommend.Offline.CheckRecommendPeriod)
		if timeoutTime.Before(time.Now()) {
...
func (w *Worker) refreshCache(ctx context.Context, userId string, recommendTime time.Time) error {
	...
	if w.Config.Recommend.Replacement.EnableReplacement {
		...
		err = w.CacheClient.AddSorted(ctx, cache.IgnoreItems, cache.Sorted(userId, items))
		if err != nil {
			return errors.Trace(err)
		}
		err = w.CacheClient.RemSortedByScore(ctx, cache.IgnoreItems, userId, math.Inf(-1), float64(recommendTime.Unix())-1)
		if err != nil {
			return errors.Trace(err)
		}
	}
